...
5. "volume": If it is a drink/beverage, recommend standard like "300 ml", else leave blank "".`;
    ...
    console.log(`[API Vercel] Gemini request received for: ${itemName} (${businessType})`);
    ...
    try {
      ...
      console.error("[API Vercel] Failed to parse Gemini response text:", bodyText);
    ...
    console.error("[API Vercel] Gemini Quick Add Error:", error);
...
5. "volume": If it is a drink/beverage, recommend standard like "300 ml", else leave blank "".`;
    ...
    console.log(`[API Vercel] Gemini bulk request received for businessType: ${businessType}`);
    const response = await ai.models.generateContent({
      model: "gemini-3.5-flash",
      contents: prompt,
      config: {
    ...
    try {
      ...
      console.error("[API Vercel] Failed to parse Gemini bulk response text:", bodyText);
      throw new Error("Unable to parse AI response.");
    }
    ...
    console.error("[API Vercel] Gemini Bulk Add Error:", error);
    res.status(500).json({ error: error?.message || "Internal generation failed" });
  }
});

// Important: Next.js / Vercel evaluates default exports here.
export default app;
